...
namespace RilixHalloweenChallenge
{
    public class RollercoasterVelocity : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            // Optionally calculate angles based on the tangent
            // Example: Pitch angle relative to the horizontal XZ plane
            float pitch = Mathf.Atan2(tangent.y, new Vector2(tangent.x, tangent.z).magnitude) * Mathf.Rad2Deg;

            acceleration = pitch * accelerationRate * Time.deltaTime * 0.01f;
            speed = Mathf.Clamp(splineFollower.followSpeed - acceleration, minSpeed, maxSpeed);
